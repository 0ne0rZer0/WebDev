...
DOM is the connection between HTML and JS : HTML modifies document and JS
modifies object models to make seemless interaction between a programming
language and markup language. It creates an OOP representation for JS.
...
In the beginning, JavaScript and the DOM were tightly intertwined, but
eventually, they evolved into separate entities. The page content is stored in
the DOM and may be accessed and manipulated via JavaScript, so that we may write
this approximative equation:
